...
def main():
    ...
    parser.add_argument('-d', '--dataset', default='VisDrone', type=str, metavar='', help='dataset')
    ...
    parser.add_argument('--gamma', default=2.5, type=float, metavar='', help='precision parameter of the Gaussian kernel (inverse of variance)')
    ...
    if args['dataset'].upper() == 'VISDRONE':
        train_data = VisDroneSeq(train=True, path=args['data_path'], size_red=args['size_red'], transform=train_transf, gamma=args['gamma'], max_len=args['max_len'])
        valid_data = VisDroneSeq(train=True, path=args['data_path'], size_red=args['size_red'], transform=valid_transf, gamma=args['gamma'], max_len=args['max_len'])
    else:
    ...
    for epoch in range(args['epochs']):
        ...
        for i, (X, density, count, _, seq_len) in enumerate(train_loader):
            ...
            seqs, seq_len = sort_seqs_by_len([X, density, count], seq_len)
            X, density, count = seqs
            # transpose them so they have shape (seq_len, batch_size, *) (as expected by the model)
            X, density, count = X.transpose(1, 0), density.transpose(1, 0), count.transpose(1, 0)

            # forward pass through the model
            density_pred, count_pred = model(X, mask=None, lengths=seq_len.cpu())
        ...
        X, density, count = None, None, None
        t0 = time.time()
        for i, (X, density, count, _, seq_len) in enumerate(valid_loader):
            # copy the tensors to GPU (if applicable)
            X, density, count, seq_len = X.to(device), density.to(device), count.to(device), seq_len.to(device)
            # sort the sequences by descending order of the respective lengths (as expected by the model)
            seqs, seq_len = sort_seqs_by_len([X, density, count], seq_len)
